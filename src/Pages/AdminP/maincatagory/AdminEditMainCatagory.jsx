...
function AdminEditMainCatagory() {
  ...
  useEffect(() => {
    (async () => {
      let response = await fetch(
        `${import.meta.env.VITE_APP_BACKEND_SERVER}/maincatagory/${id}`
      )
  ...
  function getInput(e) {
    ...
    let value =
      name === "pic"
        ? "maincatagory/" + e.target.files[0].name
        : e.target.value
...
export default AdminEditMainCatagory
