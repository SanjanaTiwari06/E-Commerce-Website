...
function AdminCreateBrand() {
    ...
    async function PostData(e) {
        ...
        if (error)
            ...
            let item = branddata.find(x=>x.name.toLocaleLowerCase()===data.name.toLocaleLowerCase())
            if(item){
                ...
                <div className="row g-4">   {/* g-0 important */}
                    ...
                    <div className="col-12 col-md-9 main-content ">
                        <h5 className="text-light p-3 text-center" style={{ background: "#ae1c9a" }}>
                            Create-Brand <Link to='/Admin/Brand'> <i className='bi bi-arrow-left fs-1 float-end text-light'></i></Link>
                        </h5>
                        <form onSubmit={PostData}>
                            <div className="row">
                                <div className="col-12 mb-3">
                                    <input type="text" name='name' placeholder='brand Name' onChange={getInput} className={`form-control mt-3 fs-4 ${Show && errormsg.name ? 'border-danger' : 'My-Border'}`} style={{ height: '40px' }} />
                                    {Show && errormsg.name ? <p className='text-danger'>{errormsg.name}</p> : null}
                                </div>
                                <div className="col-md-6 mb-2">
                                    <input type="file" name='pic' placeholder='brand Name' onChange={getInput} className={`form-control mt-3 fs-4 ${Show && errormsg.pic ? 'border-danger' : 'My-Border'}`} style={{ height: '40px' }} />
